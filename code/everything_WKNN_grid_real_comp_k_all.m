...
for i_k = 1:length(k_all)
    ...
    for layers = layer_configs
        ...
        filename = strcat(filename, "_r_real_k" , int2str(k_cur) , ".mat");
        ...
        for i_out = 1:n_round
            ...
            for k = kk
                ...
                for i = 1:n_online
                    ...
                    magic_num = round(10000000*n_online*length(kk)*1/10);
                    r = randi(magic_num);
        ...
        fprintf(fileID_z, '%s METLEARN WKNN: pearson = %f, min RMSE = %fm, (k = %d), 80th percentile = %fm, 90th percentile = %fm, grid = %d, weight factor = %f, k = %d\n', filename, pear_z, min_z, kk(i_z), val_80th_z, val_90th_z,grid, factor_z, k_cur);
    end
end

%compare to all pairwise dist training model
filepath = strcat('..\outputs\grid', int2str(grid));
for layers = layer_configs
    n_layer = length(nonzeros(layers));
    filename = strcat('_',int2str(n_layer), 'layer');
    ...
    for i_out = 1:n_round
        ...
        for k = kk
            ...
            for i = 1:n_online
                ...
                magic_num = round(9999999*n_online*length(kk)*1/10);
                r = randi(magic_num);
                if r == randi(magic_num)
                    "hit"
                    figure()
    ...
    fprintf(fileID_z, '%s METLEARN WKNN: pearson = %f, min RMSE = %fm, (k = %d), 80th percentile = %fm, 90th percentile = %fm, grid = %d, weight factor = %f\n', filename, pear_z, min_z, kk(i_z), val_80th_z, val_90th_z,grid, factor_z);
    fprintf(fileID_z, '%s EUCLDIST WKNN: pearson = %f, min RMSE = %fm, (k = %d), 80th percentile = %fm, 90th percentile = %fm, grid = %d, weight factor = %f\n', filename, pear_f, min_f, kk(i_f), val_80th_f, val_90th_f,grid, factor_f);
end
fclose(fileID_z);
