...
##########-------------------- Data Loader ------------------##########
grid = 3
offline_data = h5py.File('../pythonTrain_d_grid' + str(grid) + '_r_real' + '.mat', 'r')
...
for layers in layer_configs:
    ...
    # Testing/online data
    online_data = h5py.File('../pythonTest_d_grid' + str(grid) + '_r_real.mat', 'r')
    ...
    filename_data =  "d_grid" + str(grid) + "_" + str(n_layer) + "layer_" + '_'.join(str(x) for x in layers)
    scipy.io.savemat(filepath_data + filename_data + '_r_real' + '.mat', {'z_d': np.array(feature_d), 'phys_d': np.array(physical_d)})
